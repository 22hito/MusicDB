...
function renderSongs(){
  ...
  tbody.innerHTML=ordered.map((s,i)=>{
    ...
    return `<tr data-id="${s.id}" class="${isPlay?'playing-row':''}">
      ...
      <td class="td-mmeta m-only"><span class="mm-info">${fmtDate(s.release)} · ${esc(shortDur(s.duration))} · <svg class="icon"><use href="#icon-eye"/></svg>${s.playCount ?? 0}</span>${s.genres.slice(0,3).map(g=>`<button type="button" class="badge badge-filter${g===gf?' active':''}" data-v="${esc(g)}" onclick="filterByGenre(this.dataset.v)">${esc(abbrGenre(g))}</button>`).join('')}${s.genres.length>3?`<span class="mm-more">+${s.genres.length-3}</span>`:''}${s.album?`<button type="button" class="badge album badge-filter${s.album===albumFilter?' active':''}" data-v="${esc(s.album)}" onclick="filterByAlbum(this.dataset.v)">${esc(s.album)}</button>`:`<span class="mm-more">${t('table.single')}</span>`}</td>
